...
internal static class ProblemDetailsContextExtensions
{
    ...
    ///     [RFC 9010], Section 15.5</see>.<br/>
    ///     <br/>
    ///     ... or to pictures of cute dogs for fun and profit.
    /// </summary>
    private readonly static IReadOnlyDictionary<int, Uri> _httpClientErrorSpecificationUris = new Dictionary<int, Uri>() {
    #if DEBUG
    ...
    ///             <description>
    ///                 The default value defined in the constant <see cref="Constants.ProblemDetails.Default.Status"/>.
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <title><see cref="ProblemDetails.Title"/></title>
    ///             <description>
    ///                 The default value defined in the constant <see cref="Constants.ProblemDetails.Default.Title"/>.
    ///             </description>
    ///         </item>
    ///         <item>
    ...
    )
    {
        ...
        context.ProblemDetails.Status ??= statusCode ?? Constants.ProblemDetails.Default.Status;

        context.ProblemDetails.Title ??= title ?? Constants.ProblemDetails.Default.Title;

        context.ProblemDetails.Type ??= type ?? (
            _httpClientErrorSpecificationUris.TryGetValue(context.ProblemDetails.Status.Value, out var httpClientErrorSpecUri)
                ? httpClientErrorSpecUri
                : _httpClientErrorSpecificationUris[StatusCodes.Status400BadRequest]
        ).ToString();
    ...
    internal static void TryAddDefaultExtensions(this ProblemDetailsContext context)
    {
        ...
        context.ProblemDetails.Extensions.TryAdd(
            Constants.ProblemDetails.Extensions.RequestId,
            context.HttpContext.TraceIdentifier
        );

        context.ProblemDetails.Extensions.TryAdd(
            Constants.ProblemDetails.Extensions.TraceId,
